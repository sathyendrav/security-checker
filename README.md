...
| Source map / source leak | Scans `dist/`, `build/`, `out/`, `lib/` for source-map leakage issues (OWASP A05): **(1) sourcesContent** — flags `.map` files that embed original source code (`sourcesContent` array), meaning your entire source ships with the package; **(2) absolute local paths** — flags `sources` entries like `/Users/dev/...` or `C:\Users\dev\...` that expose developer machine topology; **(3) cloud storage URLs** — flags S3, GCS, Azure Blob, Cloudflare R2, or DigitalOcean Spaces URLs in source maps that expose internal infrastructure; **(4) sourceMappingURL** — flags built JS/CSS files that reference a `.map` file which is also present in the output, meaning both will ship to consumers |
